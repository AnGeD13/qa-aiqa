...
const testsDir = join("automation", "tests");
...
const e2eFiles = specFiles.filter((file) => file.includes(`${sep}e2e${sep}`));
const withUserSetup = e2eFiles.filter((file) => {
  const source = readFileSync(file, "utf8");
  return source.includes("registerUser") || source.includes("createCatalogHost");
});
...
console.log(
  table([
    ...
    ["Готовят пользователя", `${withUserSetup.length} из ${e2eFiles.length}`],
